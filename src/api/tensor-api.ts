...
export interface GetCollectionBidNowTxCommand{
  ownerAddress: string,
  price: number,
  quantity: number,
  collectionId: string,
...
export async function getCollectionsBy24hVolume(): Promise<CollectionResponse> {

   const response: CollectionResponse = await fetch(  
     `${baseUrl}/collections?sortBy=statsV2.volume24h%3Adesc&limit=20`,
     {
       method: 'GET',
       headers: {
         accept: 'application/json',
...
export async function getAMMBidCollectionTransaction({
  ownerAddress,
  price,
  quantity,
  collectionId,
  latestBlockhash,
}: GetCollectionBidNowTxCommand){
  try {
    const queryParams = new URLSearchParams();
    queryParams.append('collId', collectionId);
    queryParams.append('owner', ownerAddress);
    queryParams.append('poolType', "TRADE");
    queryParams.append('curveType', "EXPONENTIAL");
    queryParams.append('startingPrice', (price * 10 ** 11).toString());
    queryParams.append('delta', '0');
    queryParams.append('blockhash', latestBlockhash);
    queryParams.append('depositLamports', (price * 10 ** 9).toString());
    queryParams.append('priorityMicroLamports', '5000');
    queryParams.append('mmFeeBps','9900');
    const fullUrl = `${baseUrl}/tx/tswap/create_order?${queryParams.toString()}`;
    console.log(fullUrl);
    const response = await fetch(fullUrl, {
      method: 'GET',
      headers: {
